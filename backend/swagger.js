...
const options = {
    definition: {
        ...
        servers: [
            { url: (process.env.API_URL || 'http://localhost:5001') + '/api/v1', description: 'API v1' },
            { url: process.env.API_URL || 'http://localhost:5001', description: process.env.NODE_ENV === 'production' ? 'Production' : 'Development' }
        ],
        tags: [
