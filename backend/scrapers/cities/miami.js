...
class MiamiScraper extends BaseScraper {
    ...
    async scrape() {
        console.log('🏗️ Starting Miami scraper...');

        try {
            await this.init();
            const page = await this.newPage();

            // Use direct PDF link for Miami-Dade electrical fee schedule
            const pdfLink = 'https://www.miamidade.gov/permits/library/fees/electrical-fee-sheet.pdf';
            ...
            // Save PDF
            const pdfPath = await this.savePDF(pdfBuffer, 'miami-fee-schedule-2025.pdf');
            ...
            const result = {
                ...
                source: 'Miami Building Department Fee Schedule',
                sourceUrl: pdfLink,
            ...
            console.log('✅ Miami scraping complete');
            return result;

        } catch (error) {
            console.error(`❌ Miami scraper failed: ${error.message}`);
            await this.close();
            throw error;
        }
    }
...
if (require.main === module) {
    (async () => {
        const scraper = new MiamiScraper();
        try {
